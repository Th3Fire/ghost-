...
CGHost :: CGHost( CConfig *CFG )
{
	...
	//m_SimpleAFKScript = CFG->GetInt( "bot_afkdetect", 1) == 0 ? false : true; //code by wuttinunt
	...
	if( m_AdminGameCreate )
	{
		...
		m_AdminGame = new CAdminGame( this, m_AdminMap, NULL, m_AdminGamePort, 0, "|cFF35FFFBGHost++ Server By Tae", m_AdminGamePassword );
...
void CGHost :: SetConfigs( CConfig *CFG )
{
	...
	m_VoteEndAllowed = CFG->GetInt( "bot_voteendallowed", 1 ) == 0 ? false : true;  // code by wuttinunt
	m_AfkDetection = CFG->GetInt( "bot_afkdection", 1 ) == 0 ? false : true;  // code by wuttinunt
