...
bool CGame :: Update( void *fd, void *send_fd )
{
	...
	if(!m_GameLoading && !m_GameLoaded && GetTime( ) - m_TimeGetLastCR  >= 20 )
	{
		m_TimeGetLastCR = GetTime();
		m_b = m_b-20;
		if(m_a == 0)
		{
			if(m_b == 0)
			{
				SendAllChat("Time out closed Lobby");
				m_checkLimit = true;
		...
		SendAllChat(m_GHost->m_Language->TimeCountDownLimitLobby( UTIL_ToString(m_a),UTIL_ToString(m_b) ) );
		//SendAllChat("Please start game within " + UTIL_ToString(m_a) + " mins. " + UTIL_ToString(m_b)+ " sec.");
		//SendChat( Player, "Please start game within " + UTIL_ToString(m_a) + " mins. " + UTIL_ToString(m_b)+ " sec.");
...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
			...
			if( Command == "sendlan" && !Payload.empty( ) && !m_CountDownStarted )
			{
				...
				else
				{
					...
					if( m_SaveGame )
					{
						...
						MapHeight.push_back( 0 );
						m_GHost->m_UDPSocket->SendTo( IP, Port, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, UTIL_CreateByteArray( MapGameType, false ), m_Map->GetMapGameFlags( ), MapWidth, MapHeight, m_GameName, "GHost++", GetTime( ) - m_CreationTime, "Save\\Multiplayer\\" + m_SaveGame->GetFileNameNoPath( ), m_SaveGame->GetMagicNumber( ), 12, 12, m_HostPort, m_HostCounter ) );
					}
					else
					{
						// note: the PrivateGame flag is not set when broadcasting to LAN (as you might expect)
						...
						m_GHost->m_UDPSocket->SendTo( IP, Port, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, UTIL_CreateByteArray( MapGameType, false ), m_Map->GetMapGameFlags( ), m_Map->GetMapWidth( ), m_Map->GetMapHeight( ), m_GameName, "GHost++", GetTime( ) - m_CreationTime, m_Map->GetMapPath( ), m_Map->GetMapCRC( ), 12, 12, m_HostPort, m_HostCounter ) );
					}
				}
			}
	...
	*********************/
	...
	if(Command == "voteend" || Command == "regame" && m_GameLoaded)
	{
		...
		string EndVotedPlayer;
		std::list<string> mylist;
		string myint;

		if(m_VoteEndCheck == false)
		{
			...
			for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); ++i )
            {
                ...
                {
                    ++Votes;
                    if( !EndVotedPlayer.empty( ) )
                    EndVotedPlayer+=", "+(*i)->GetName();
			...
			SendAllChat(m_GHost->m_Language->UserStartVoteEnd( User ) );
			SendChat( player, "votes ["+UTIL_ToString(Votes)+"/ "+UTIL_ToString(VotesNeeded)+"] need ["+ UTIL_ToString(VotesNeeded - Votes)+"]");

			//SendAllChat( m_OHBot->m_Language->UserStartedVoteEndNotify( ) );
			m_VoteEndCheck = true;


		}else if(Payload.empty() && m_VoteEndCheck)
		{	
            ...
            SendChat( player, EndVotedPlayer + " votes.");
            SendChat( player, "votes ["+UTIL_ToString(Votes)+"/ "+UTIL_ToString(VotesNeeded)+"] need ["+ UTIL_ToString(VotesNeeded - Votes)+"]");
            //SendAllChat("voted > " + UTIL_ToString(Votes) + " Need > " +UTIL_ToString(VotesNeeded));

			if(Votes >= VotesNeeded)
			{
				
					StopPlayers( "Vote end passed stop game)" );
